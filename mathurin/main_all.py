...
# for a in values:
#     for b in values:
with open(f"results_1111.txt", "w") as file:
    # file.write("=============================\n")
    ...
    for c in range(total_classes):
        if c < nb_cl_first:
            arrival_task.append(0)  # normalement 0, mais pour tester en lui mettant autant d'examples que la seconde tâche
        else:
            arrival_task.append(1 + (c - nb_cl_first) // nb_cl)  # task où la classe arrive
    ...
    with open(f"{nb_cl}_files_protoset0.pickle", "rb") as f:
        files_protoset = pickle.load(f)

    # GPU deterministic settings
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False

    # order1 = np.arange(total_classes)
    # mixing1 = np.arange(total_classes)
    # np.random.shuffle(mixing1)

    ## load the previous orders
    with open(f"{nb_cl}mixing.pickle", "rb") as f:
        mixing = pickle.load(f)

    with open(f"{nb_cl}settings_mlp.pickle", "rb") as f:
        order = pickle.load(f)
        files_train = pickle.load(f)
        files_valid = pickle.load(f)
    ...
    model0 = EmberTransformerPyTorch(**config.model_config).to(device)
    ckpt0 = save_path + f"model-iteration{nb_cl}-0.pt"
    model0.load_state_dict(torch.load(ckpt0, map_location=device))
    model0.eval()
    ...
    end_class0 = nb_cl_first  # at itera=0 you only know nb_cl_first classes

    eval0 = utils_eval.evaluate_model_performance(
        model0, X_test, y_test, nb_cl_first, nb_cl, itera=0, end_class=end_class0
    )

    task_accuracy_all.append(eval0['task_accuracies'].copy())
    task_best_acc_list.append(max(eval0['task_accuracies']))

    memory= files_protoset[:22]#[b for b in files_protoset if b]



    ##### ------------- Main Algorithm START -------------#####
    for itera in range(resume_from_iter, nb_groups + 1):
        # a=2*itera
        # b=itera/2
        # file.write(f"a= {a}, b= {b}\n")
        ...
        if itera == 0:
            ...
            training, future_train= utils_data.split_classes(files_train, y_path, mixing, itera, seed=42)
            ...
            for i in range(total_cl_now): 
                tmp_var = random.sample(memory[i],min(len(memory[i]),nb_protos_cl))
                idx_iter += tmp_var
            
            for classe in future_train.keys():
                memory.append(future_train[classe])
        ...
        print(f"on task {itera}, memory size is {len(memory)}")
        ...
        for epoch in range(epochs):
            ...
            for step, (x_batch, y_batch) in enumerate(train_loader):
                ...
                if (step + 1) % 50 == 0:
                    ...
                    # file.write(f"\r, {epoch}, epoch, {step}, batch, Average loss: {np.mean(epoch_losses[-100:]):.6f},")
            ...
            file.write(f"epoch: {epoch}, Val_Loss: {avg_val_loss:.4f}, Val_Acc: {val_acc:.4f}, LR: {current_lr:.6e}\n")
        ...
        # selected_exemplars_files = utils_exemplar.mean_exemplar_selection(D, files_iter, nb_protos_cl, method='random')    
            ...
            if itera == 0:
                with open(f"{nb_cl}_files_protoset0.pickle", "wb") as fp:
                    pickle.dump(files_protoset, fp)
...
############### For an adaptative number of examples ####################
            # ages= np.array([itera - arrival_task[c] for c in range(total_cl_now)], dtype= float)
            # gamma= 0.15 #this parameter decides on how much more samples we are using for the oldest class compare to the newest
            # weights= np.exp(gamma * ages)
            # raw= weights / weights.sum() * nb_total
            # quotas= np.ceil(raw).astype(int)
            # print(f"selection done with a gamma {gamma}")
##############################################################################
            ...
            for class_id, exemplar_list in enumerate(files_protoset):
                if len(exemplar_list) > 0:
                    if class_id < (nb_cl_first + itera * nb_cl):
                        ...
                        # selected_exemplars= random.sample(exemplar_list,k=min(len(exemplar_list), quotas[class_id]))
            ...
            if len(all_exemplar_indices) > 0:
                ...
                utils_exemplar.run_exemplar_fine_tuning(
                    ...
                    exemplar_epochs, loss_fn, ex_opt, start_idx, device, a=4, b=1
                )
            else:
    ...
    utils_eval.print_results_summary(iteration_results, nb_groups, file)
    finale_accuracy= task_accuracy_all[-1]
    mean_accuracy= np.mean(finale_accuracy)
    ...
    if forgetting_scores:
        ...
        # print(f"finale forgetting score: {forgetting_scores[-1]:.4f}\n")
        print(f"Final task best accuracies: {[f'{acc:.4f}' for acc in task_best_acc_list]}")
        print("Final task accuracies:", finale_accuracy)
        print("accuracies: ", task_accuracy_all)
        print("Mean Accuracy:", mean_accuracy)

        file.write(f"Forgetting scores per iteration: {[f'{f:.4f}' for f in forgetting_scores]}\n")
        file.write(f"Average forgetting score: {np.mean(forgetting_scores):.4f}\n")
        # file.write(f"finale forgetting score: {forgetting_scores[-1]:.4f}\n")
        file.write(f"Final task best accuracies: {[f'{acc:.4f}' for acc in task_best_acc_list]}\n")
        file.write(f"Final task accuracies: {finale_accuracy}\n")
        file.write(f"accuracies: {task_accuracy_all}\n")
