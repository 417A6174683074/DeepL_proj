...
def print_results_summary(iteration_results, nb_groups, f):
    ...
    for itera_num in range(1, nb_groups + 1):
        ...
        if before_result:
            ...
            f.write(f"  Task {itera_num+1} (Iter {itera_num+1}) - Before: {before_result['overall_accuracy']:.4f}\n")
        if after_result:
            print(f"  Task {itera_num+1} (Iter {itera_num+1}) - After:  {after_result['overall_accuracy']:.4f}")
            f.write(f"  Task {itera_num+1} (Iter {itera_num+1}) - After:  {after_result['overall_accuracy']:.4f}\n")
